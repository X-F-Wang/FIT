...
def prepare_training():
    if config.get('pre_train') is not None:
        ...
        if config.get('multi_step_lr') is None:
            lr_scheduler = CosineAnnealingLR(optimizer, config['epoch_max']-config['warmup_step_lr']['total_epoch'])
        else:
            lr_scheduler = MultiStepLR(optimizer, **config['multi_step_lr'])

        lr_scheduler.last_epoch = epoch_start - config['warmup_step_lr']['total_epoch'] #防止warmup影响lr
        ...
        if config.get('multi_step_lr') is None:
            # eta_min = config['warmup_step_lr'].pop('eta_min')
            cosine = CosineAnnealingLR(optimizer, config['epoch_max']-config['warmup_step_lr']['total_epoch'])
            lr_scheduler = GradualWarmupScheduler(optimizer,**config['warmup_step_lr'],after_scheduler=cosine)
        else:
            lr_scheduler = MultiStepLR(optimizer, **config['multi_step_lr'])
...
def main(config_, save_path):
    ...
    arch_path_ = './models'   #把models复制到保存路径中
    arch_path = os.path.join(arch_path_, '{}.py'.format(config['model']['name']))
    save_path_ = '{}/{}.py'.format(save_path,config['model']['name'])
    shutil.copyfile(arch_path,save_path_)

    arch_path_ = '.'   #把models复制到保存路径中
    arch_path = os.path.join(arch_path_, '{}.py'.format("test_mod"))
    save_path_ = '{}/{}.py'.format(save_path,"test_mod")
    shutil.copyfile(arch_path,save_path_)
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        log_info.append('lr:{:.8f}'.format(optimizer.param_groups[0]['lr']))
        ...
        with torch.no_grad():
            if (epoch_val is not None) and (epoch % epoch_val == 0):
                ...
                for idx in range(1, len(val_res)):
                    ...
                    # log_info.append('best: psnr={:.4f}'.format(max_vals[idx]))
                    # writer.add_scalars('best_psnr', {'val': max_vals[idx]}, epoch)
