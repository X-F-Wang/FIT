...
from mod import RP,FIM,FIM1,FCSA,IISA
...
class FIT(nn.Module):
    def __init__(
        ...
        self.FCSA = FCSA(dim=256)
        self.FIM= FIM(dim=64)
        self.FIM1 = FIM1(dim=64)
        self.gelu = nn.GELU()

        self.encoder = models.make(encoder_spec)

        self.conv_ch = nn.Conv2d(self.dim//4, self.dim, kernel_size=3, padding=1)
        ...
        if self.local_attn:
            ...
            self.multi_conv_qs = nn.ModuleList([
                IISA(midc=self.dim, num=self.MsSA_q)
    ...
    def gen_feat(self):
        if self.prev_feat is None:
            ...
            self.feat0 = self.feat
            self.feat = self.FIM(self.feat)
            self.feat = self.feat+self.feat0

            self.featz = self.FIM1(self.feat)

            self.feat = self.conv_ch(self.feat)

            self.Feat = self.FCSA(self.feat)
        ...
        if self.local_attn:
            ...
            self.feat_q = self.multi_conv_qs[self.conv_idx](self.featz)
    ...
    def query_rgb(self, sample_coord, cell=None):
        ...
        if self.local_attn:
            ...
            sample_feat_k = F.grid_sample(
                self.feat_q, sample_coord_k.flip(-1), mode='nearest', align_corners=False
            ).permute(0, 2, 3, 1)
            sample_feat_k = sample_feat_k.reshape(
                bs, q_sample, self.r_area, self.head, self.dim // self.head
            ).permute(0, 1, 3, 4, 2)

        sample_feat_f = F.grid_sample(
            self.Feat, sample_coord_k.flip(-1), mode='nearest', align_corners=False
        ).permute(0, 2, 3, 1)
        ...
        if self.local_attn:
            ...
            sample_feat_v = sample_feat_v + sample_feat_f
